...
//		$seed_settings_page = add_submenu_page ( 'themes.php', __( 'Seed Settings', 'seed-settings' ), __( 'Settings', 'seed-settings' ), 'manage_options', 'seed-settings', array( $this, 'license_page' ) );

		$hookname = get_plugin_page_hookname( 'seed-settings', 'themes.php' );
		add_action( $hookname, array( $this, 'license_page' ) );
	}
	...
	function license_page() {
		...
		?>
		<div class="wrap">
		<?php seed_settings_page_tabs( $tab ); ?>
	...
	function activate_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( false === $license_data->success ) {
				...
				if ( ! empty( $message ) ) {
					$base_url = admin_url( 'themes.php?page=seed-settings&tab=license' );
...
//		wp_redirect( admin_url( 'themes.php?page=' . $this->theme_slug . '-license' ) );
		...
		wp_redirect( admin_url( 'themes.php?page=seed-settings&tab=license' ) );
